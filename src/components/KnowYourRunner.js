...
const KnowYourRunner = ({
  ...
  const fetchPendingActionRequests = async () => {
    try {
      ...
      // Separate transfers and defers
      const transferEmails = new Set();
      const deferEmails = new Set();

      data?.forEach(request => {
        if (request.action_type === 'Transfer Runner') {
          transferEmails.add(request.runner_email_id);
        } else if (request.action_type === 'Defer Runner') {
          deferEmails.add(request.runner_email_id);
        }
      });

      setPendingTransfers(transferEmails);
      setPendingDefers(deferEmails);
    } catch (error) {
      console.error('Error in fetchPendingActionRequests:', error);
    }
  };
  ...
  const toggleSection = (sectionName) => {
    ...
    // If opening metrics section and we have a selected runner, fetch the data
  ...
  const validateOverriddenScore = (value) => {
  ...
  // Fetch season metrics for a specific runner
  const fetchSeasonMetrics = async (runnerEmail) => {
    if (!runnerEmail || !currentSeason || !coachEmail) return;
    ...
    try {
      // Execute query to fetch meso metrics
      const { data, error } = await supabase
      ...
      if (error) {
        ...
        setSeasonMetrics(data);
      }
    } catch (error) {
      console.error('Error in fetchSeasonMetrics:', error);
      setSeasonMetrics(null);
    } finally {
  ...
  const handleEditMeso = (mesoData) => {
    ...
    setEditFormData({
      meso_score_override: mesoData.meso_score_override || '',
      meso_qual_score: mesoData.meso_qual_score || ''
    });
  ...
  const handleEditFormChange = (field, value) => {
    ...
    // Validate overridden score in real-time
    if (field === 'meso_score_override') {
      const validation = validateOverriddenScore(value);
      if (!validation.isValid) {
        setEditValidationErrors(prev => ({ ...prev, [field]: validation.message }));
      }
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (!overrideValidation.isValid) {
      setEditValidationErrors(prev => ({ ...prev, meso_score_override: overrideValidation.message }));
      return;
    }
    ...
    try {
      ...
      if (error) {
        console.error('Error updating meso metrics:', error);
        setEditMessage({ type: 'error', text: 'Failed to update metrics. Please try again.' });
        return;
      }
  ...
  const handleRunnerSelect = (runner) => {
    ...
    if (selectedRunner?.email_id === runner.email_id) {
      ...
      setExpandedSections({});
      setSeasonMetrics(null); // Clear metrics when deselecting
    } else {
                    ...
                    <div className="flex items-center space-x-2 sm:space-x-4 min-w-0 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3">
                          ...
                          {/* Pending Transfer/Defer Status */}
                          {(pendingTransfers.has(runner.email_id) || pendingDefers.has(runner.email_id)) && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200">
                              {pendingTransfers.has(runner.email_id) ? 'Pending Transfer' : 'Pending Defer'}
                            </span>
                          )}
                          ...
                          {runner.phone_no && (
                            <div className="hidden sm:flex items-center text-xs sm:text-sm text-gray-500 space-x-2">
                              ...
                              <button
                                onClick={() => copyToClipboard(runner.phone_no, 'phone', runner.email_id)}
                                className="text-gray-400 hover:text-gray-600 transition-colors"
                                title="Copy phone number"
                              >
                                {copiedItems[`${runner.email_id}-phone`] ? (
                        ...
                        <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1">
                          ...
                          {runner.email_id && (
                            <div className="hidden sm:flex items-center text-xs sm:text-sm text-gray-500 space-x-2">
                              ...
                              <button
                                onClick={() => copyToClipboard(runner.email_id, 'email', runner.email_id)}
                                className="text-gray-400 hover:text-gray-600 transition-colors"
                                title="Copy email address"
                              >
                                {copiedItems[`${runner.email_id}-email`] ? (
                                  <Check className="h-3 w-3 sm:h-4 sm:w-4" />
                                ) : (
                                  <Copy className="h-3 w-3 sm:h-4 sm:w-4" />
                                )}
                    ...
                    <div className="absolute top-full left-0 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
                      {['5K', '10K', 'Half Marathon', 'Full Marathon'].map((distance) => (
                        <button
                          key={distance}
                          onClick={() => {
                            handleProgramSelection(distance);
                            setTransferDistanceMenuOpen(false);
                          }}
